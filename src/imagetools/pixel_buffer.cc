...
void PixelBuffer::SaveToFile(const std::string &filename) {
  // utilizing the imageio
  imageio::Image image = imageio::Image(width_, height_, 4);

  for (int i = 0; i < width_; i++) {
    for (int j = 0; j < height_; j++) {
      pixel(i, j).Clamp();  // clamp the pixel
      image.SetFloatValue(i, j, 0, pixel(i, j).red());  // set red
      image.SetFloatValue(i, j, 1, pixel(i, j).green());  // set green
      image.SetFloatValue(i, j, 2, pixel(i, j).blue());  // set blue
      image.SetFloatValue(i, j, 3, pixel(i, j).alpha());  // set alpha
    }
  }
  // save to file by calling imageio
...
void PixelBuffer::LoadFromFile(const std::string &filename) {
  ...
  Resize(image->Width(), image->Height());  // control the resize
  float r, g, b, a;
  for (int i = 0; i < width_; i++) {
    for (int j = 0; j < height_; j++) {
      r = image->FloatValue(i, j, 0);  // getting the float value on imageio
      g = image->FloatValue(i, j, 1);
      b = image->FloatValue(i, j, 2);
      a = image->FloatValue(i, j, 3);

      ColorData pixel = ColorData(r, g, b, a);
      pixel.Clamp();  // clamp the pixel
      set_pixel(i, j, pixel);
    }
  }
  delete image;
